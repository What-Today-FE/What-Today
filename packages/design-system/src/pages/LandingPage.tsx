...
디자인 시스템의 컴포넌트 문서를 작성할 때는, **\`pnpm generate-doc\`** 명령어를 사용하여 자동으로 컴포넌트와 문서 템플릿을 생성할 수 있습니다.  
명령어 실행 후, 컴포넌트 이름을 입력하면 아래의 작업들을 자동으로 처리합니다.

1. 컴포넌트 파일 생성 (ex. \`src/components/Button.tsx\`) 
2. 문서 페이지 파일 생성 (ex. \`src/pages/ButtonDoc.tsx\`)
3. 라우터 등록
4. 사이드바에 링크 추가
...
- 겹치는 컴포넌트 이름이 있을 경우, 기존 문서와 컴포넌트가 덮어쓰여질 수 있습니다.
- **실제 서비스에서 개발 중 디자인 시스템 컴포넌트의 업데이트가 생기면, \`pnpm build-ui\` 명령어를 실행하여 변경 사항을 반영해야 합니다.**
...
6. **Playground (편집 가능한 코드 블록)**  
   \`<Playground>\` 컴포넌트를 사용하여 사용자가 직접 코드를 편집하고 결과를 확인할 수 있는 영역을 제공합니다.  
   이 영역은 컴포넌트의 사용법을 실시간으로 테스트할 수 있는 공간입니다.    
